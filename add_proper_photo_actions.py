#!/usr/bin/env python3
"""
Script to add properly formatted photo capture actions to all waypoints in a WPML file.
This script uses the exact same formatting as the original waypoints 2 and 4.
"""

import xml.etree.ElementTree as ET
from pathlib import Path

def add_proper_photo_actions(input_file, output_file=None):
    """
    Add properly formatted photo capture actions to all waypoints that don't already have them.
    Uses the exact same formatting as the original waypoints 2 and 4.
    """
    ...
    # Track which waypoints already have properly formatted photo actions
    ...
    # Add properly formatted photo actions to waypoints that don't have them
    ...
    for placemark in placemarks:
        ...
        if waypoint_index is not None:
            ...
            if index not in waypoints_with_photo_actions:
                print(f"Adding properly formatted photo action to waypoint {index}")
                
                # Create action group for photo capture with proper formatting
    ...
    # Use pretty printing to ensure proper indentation
...
if __name__ == "__main__":
    ...
    output_file = "waylines_proper_photos.wpml"  # Create a new file first
    
    print("Adding properly formatted photo capture actions to all waypoints...")
    print(f"Input file: {input_file}")
    print(f"Output file: {output_file}")
    print("-" * 60)
    
    add_proper_photo_actions(input_file, output_file)
